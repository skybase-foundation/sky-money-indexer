...
- **Framework**: Envio HyperIndex v3 (TypeScript handlers, ESM — `"type": "module"`, Node 22+)
...
- **Handlers**: `src/*.ts` — One file per contract/feature. Registered via `indexer.onEvent()` from `"envio"`
- **Helpers**: `src/helpers/` — Shared logic (entity creation, delegation handlers, contract calls via viem)
- **Contract calls**: `src/helpers/contractCalls.ts` — On-chain reads using `viem` via `createEffect` (replaces The Graph's `Contract.bind()`)
- **Dynamic contracts**: Delegate factories use `indexer.contractRegister` for runtime contract registration
- **Generated code**: `.envio/types.d.ts` — Auto-generated by `pnpm codegen` (referenced via `envio-env.d.ts`), do not edit
...
### Types
Generated types are imported from `"envio"` (module augmentation via codegen):
- `EvmEvent<'Contract', 'Event'>` — event types (v3 events carry a literal `contractName`, so helpers shared across contracts need union types)
- `Entity<'EntityName'>` — entity types
- `EvmOnEventContext` — handler context type
- `BigDecimal` — value export
